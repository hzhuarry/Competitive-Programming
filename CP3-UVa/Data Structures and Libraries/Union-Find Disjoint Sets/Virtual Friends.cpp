...
void solve(){
    ...
    map<str,int> m;
    int i=0;
    //assign an index to each name
    fitfor(it,s){
        if(m.find(*it)==m.end()||m.empty()){
            m[*it]=i++;
        }
    }
    DSU D; D.init(i);
    //process queries
    FOR(n){
        D.merge(m[queries[i].f],m[queries[i].s]);
        cout<<D.get_sz(m[queries[i].f])<<endl;
    }
}
