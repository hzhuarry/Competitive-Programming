...
ll query(int l,int r, int index=0,int leftb=0,int rightb=N-1){
    if(l>rightb||r<leftb) return 0;
    if(l<=leftb && r>=rightb){
        return seg[index];
    }
    int mid=(leftb+rightb)/2;
    return query(l,r,2*index+1,leftb,mid) + query(l,r,2*index+2,mid+1,rightb);
}

void upd(int i, int val, int index=0, int leftb=0,int rightb=N-1){
    if(leftb==rightb){
        seg[index]=val;
    }
    else{
        int mid=(leftb+rightb)/2;
        if(i<=mid){
            upd(i,val,2*index+1,leftb,mid);
        }
        else{
            upd(i,val,2*index+2,mid+1,rightb);
        }
        seg[index]=seg[2*index+1] + seg[2*index+2];
    }
}
