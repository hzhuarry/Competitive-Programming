...
int main() {
    ...
    read(N);
    FOR(N){
        int start,finish; read(start,finish);
        s.ins({start,i});
        s.ins({finish,i});
        lifeguards.pb({start,finish});
    }
    set<int> curr_shift;
    //keep track of each lifeguards points
    vi points(N,0);
    int t=0;
    fitfor(it,s){
        int currTime=it->f;
        int currLifeGuard=it->s;
        if(sz(curr_shift)==1){
            points[*curr_shift.begin()]+=abs(t-currTime);
        }
        t=currTime;
        if(curr_shift.find(currLifeGuard)!=curr_shift.end()){
            curr_shift.erase(currLifeGuard);
        }
        else{
            curr_shift.ins(currLifeGuard);
        }
    }
    int smallIndex= distance(points.begin(),min_element(all(points)));
    lifeguards.erase(lifeguards.begin()+smallIndex);
    sort(all(lifeguards),cmp);
    t=0;
    //now we count the time
    trav(i,lifeguards){
        int start=i.f,end=i.s;
        if(end<t) continue;
        if(start<t){
            ans+=end-t;
        }
