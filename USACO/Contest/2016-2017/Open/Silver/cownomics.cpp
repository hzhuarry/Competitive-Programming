...
int N,M,ans;
int spotty[500][50], plain[500][50];
...
bool ok(int a,int b,int c){
    ...
    for(int i=0;i<N;++i){
        //4^0, 4^1, 4^2
        vis[spotty[i][c] + 4*spotty[i][b] + 16*spotty[i][a]] = 1;
    }
    for(int i=0;i<N;++i){
        if(vis[plain[i][c] + 4*plain[i][b] + 16*plain[i][a]])
            return 0;
    }
    return 1;
}
int main() {
    ...
    for(int i=0;i<N;++i){
        cin >> s;
        for(int j=0;j<M;++j){
            spotty[i][j]=convert(s[j]);
        }
    }
    for(int i=0;i<N;++i){
        cin >> s;
        for(int j=0;j<M;++j){
            plain[i][j]=convert(s[j]);
        }
    }
    for(int i=0;i<M;++i){
        for(int j=i+1;j<M;++j){
