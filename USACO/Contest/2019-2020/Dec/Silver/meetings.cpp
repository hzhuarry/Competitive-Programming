...
vi leftTimes,rightTimes;
vector<ar<int,2> > times;
vi leftX;
...
int main() {
    ...
    FOR(N){
        //sort by x
        read(cows[i][1],cows[i][0],cows[i][2]);
        totalWeight+=cows[i][1];
    }
    Sort(cows);
    trav(t,cows){
        if(t[2]==-1){
    ...
    for(int i=0;i<sz(leftTimes);++i){
        times.pb({leftTimes[i],cows[i][1]});
    }
    reverse(all(rightTimes));
    for(int i=0;i<sz(rightTimes);++i){
        times.pb({rightTimes[i],cows[N-i-1][1]});
    }
    Sort(times);
    int currTotal=0, index=0;
    while(currTotal * 2<totalWeight){
        currTotal+=times[index][1];
        ++index;
    }
    int T = times[index-1][0];
    Sort(leftX);
    int ans=0;
    trav(i,cows){
        if(i[2] == 1){
            ans+= ub(all(leftX),i[0] + 2*T) - lb(all(leftX), i[0]);
        }
    }
    cout<<ans<<endl;
    return 0;
