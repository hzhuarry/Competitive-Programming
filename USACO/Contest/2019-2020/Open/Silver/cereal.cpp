...
int N,M;
int firstChoice[100000],secondChoice[100000];
int cereal[100001];
vi ans;
...
int main() {
    ...
    read(N,M);
    FOR(N){
        read(firstChoice[i],secondChoice[i]);
    }
    int cnt=0;
    //process backwards
    FOR(i,N,0){ 
        int currChoice=firstChoice[i];
        int currIndex=i;
        while(1){
            if(cereal[currChoice]==0){
                cereal[currChoice]=currIndex;
                cnt++;
                break;
            }
            else if(cereal[currChoice]>currIndex){
                int ni= cereal[currChoice];
                cereal[currChoice]=currIndex;
                if(secondChoice[ni]==currChoice)break;
                currChoice=secondChoice[ni];
                currIndex=ni;
            }
            else{
                break;
