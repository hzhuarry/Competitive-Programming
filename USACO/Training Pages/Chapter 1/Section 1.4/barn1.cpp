...
int main() {
    ...
    Sort(cows);
    //M-1 gaps
    //we want the largest gaps possible between the boards - greedy algo
    vi gaps;
    for(int i=1;i<C;++i){
        int g=cows[i]-cows[i-1]-1;
        gaps.pb(g);
    }
    sort(all(gaps),greater<int>());
    int totalBetweenGaps=0;
    for(int i=0;i<M-1&&i<sz(gaps);++i){
        totalBetweenGaps+=gaps[i];
    }
    int totalSpaces=cows[C-1]-cows[0]+1;
    cout<<totalSpaces-totalBetweenGaps<<endl;
    return 0;
}
